 ...
 * — the same shape rotation-reset-hotkey.ts uses for Board-scoped Alt+R.
 * `setHotkeys` overwrites the custom-keys store for a command
 ...
 * conflicts; `bake()` recomputes them after `setHotkeys` invalidates them.
 * This is an undocumented Obsidian interface and must be re-checked on upgrade.
 */
