...
A Popout hides both layers of chrome: Obsidian's outer chrome (ribbon, tabs, sidebars, status bar) and Excalidraw's own in-canvas UI (toolbar, style panel, zoom controls, library panel) — nothing but the bare drawn canvas is visible. The host plugin also enables Excalidraw's own Zen Mode for the Popout view once the canvas API is live via Obsidian Excalidraw's view-level `updateScene`; direct `excalidrawAPI.updateScene` does not apply Zen Mode in the Obsidian wrapper. This matches PureRef's minimal surface without changing the originating main-window view. All interaction is hotkey/mouse-driven (paste, drag-drop, scroll-zoom, click-drag pan, right-click context menu); no on-canvas UI is ever shown for this feature's own configuration.
...
Optionally (the **"Transparent (see-through) popout"** setting, off by default) a Popout is opened as a genuinely see-through, frameless OS window — PureRef's signature look, where the board's images float over whatever is behind the window. This is real per-pixel window transparency, not a blurred/translucent backdrop. Because Electron window transparency is fixed at window-creation time and can only be requested by a synchronous main-process `setWindowOpenHandler`, the host plugin runs a tiny main-process helper (`electron-main-helper.cjs`, loaded via `@electron/remote.require`) that tags only the plugin's own Popout as `transparent + frameless` for the instant it is created, leaving all other Obsidian windows untouched. Being frameless (transparency is frameless-only on Windows), the Popout also hides Obsidian's in-page title bar and relies on the existing RMB window-drag and F11 close. The board's Excalidraw `viewBackgroundColor` is forced transparent while such a Popout is open (otherwise the canvas would paint over the transparency) and restored to its prior value on close. Windows desktop only; degrades to an opaque Popout if the Electron bridge is unavailable. See ADR 0007 (which supersedes ADR 0003's "keep the native title bar" decision).
_Avoid_: translucency/vibrancy/Mica/Acrylic (a blurred system backdrop — a different, non-goal effect), "transparent mode is the default" (it is opt-in, off by default)
